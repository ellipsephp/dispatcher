...
// The request goes through SomeMiddleware1, SomeMiddleware2 then the fallback response is returned when none of them return a response on its own.
$response = $dispatcher->handle($request);
```
